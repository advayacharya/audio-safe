# File: app.py
from flask import Flask, request, jsonify, render_template, send_file, flash, redirect, url_for, abort, send_from_directory
import json
import os
import io
import time
...
# --- CONFIG ---
app.config['SECRET_KEY'] = 'fixed-key-for-pythonanywhere-deployment-123'
...
# --- CRYPTO ---
KEY_SIZE_BYTES, SALT_SIZE_BYTES, ITERATIONS = 32, 16, 1000 
SYNC_WORD = b"SYNC"
...
def extract_data_stream(file_stream):
    ...
    if len(header) < 12 or header[0:4] != b'RIFF' or header[8:12] != b'WAVE': return None
    while True:
        chunk_header = file_stream.read(8)
        if len(chunk_header) < 8: return None 
        chunk_id = chunk_header[0:4]
        chunk_size = int.from_bytes(chunk_header[4:8], 'little')
        if chunk_id == b'data': break
        else:
            file_stream.seek(chunk_size, os.SEEK_CUR)
            if chunk_size % 2 == 1: file_stream.seek(1, os.SEEK_CUR)
    sync_bytes_audio = file_stream.read(32)
    if len(sync_bytes_audio) < 32: return None
    sync_bits = ""
    for byte in sync_bytes_audio: sync_bits += str(byte & 1)
    target_sync = ''.join(format(b, '08b') for b in SYNC_WORD)
    if sync_bits != target_sync: return None
    len_bytes_audio = file_stream.read(32)
    if len(len_bytes_audio) < 32: return None
    len_bits = ""
    for byte in len_bytes_audio: len_bits += str(byte & 1)
    try: payload_len = int(len_bits, 2)
    except: return None
    payload_bits_count = payload_len * 8
    if payload_len > 20 * 1024 * 1024: raise ValueError("Hidden message is too large.")
...
def dashboard():
    ...
    return render_template('dashboard.html', secrets=user_secrets)
...
def encode_text():
    ...
    try:
        ...
        filename = f"secure_{int(time.time())}_{cover_file.filename}"
        save_path = os.path.join(downloads_dir, filename)
        with open(save_path, 'wb') as f:
            f.write(stego_bytes)
            
        try:
            new_log = SavedSecret(title=f"Encoded: {cover_file.filename}", encrypted_content="HIDDEN", owner=current_user)
            db.session.add(new_log)
            db.session.commit()
        except: pass

        # CHANGED: Render Success Page instead of redirecting
...
def encode_image():
    ...
    try:
        ...
        filename = f"secure_img_{int(time.time())}_{cover_file.filename}"
        save_path = os.path.join(downloads_dir, filename)
        with open(save_path, 'wb') as f:
            f.write(stego_bytes)

        # CHANGED: Render Success Page
        return render_template('success.html', filename=filename)
    ...
    try:
        ...
        if header == b"TEXT":
            ...
            try:
                ...
                return jsonify({'type': 'text', 'content': content, 'analytics': {'extraction_time': round(ext_time, 6), 'decryption_time': round(dec_time, 6)}})
            except: return jsonify({'error': 'Decryption failed.'}), 400
        elif header == b"IMG_":
            return jsonify({'type': 'image', 'file_b64': base64.b64encode(payload).decode('ascii'), 'filename': 'decoded_image.png', 'analytics': {'extraction_time': round(ext_time, 6)}})
        else: return jsonify({'error': 'Unknown format'}), 400
    except ValueError as ve: return jsonify({'error': str(ve)}), 400
    except Exception as e: return jsonify({'error': "Processing failed."}), 500
...
def delete_secret(id):
    ...
    if secret.owner.id != current_user.id: abort(403) 
