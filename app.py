...
# --- CRYPTO & STEGO ---
KEY_SIZE_BYTES, SALT_SIZE_BYTES, ITERATIONS = 32, 16, 2000 
SYNC_WORD = b"SYNC"
...
def encode_text():
    ...
    try:
        # 2. Encrypt (Fast)
        # Note: We keep iterations low (1000) for speed on free tier
        ...
        # 4. Hide Data (Steganography)
        stego_bytes = hide_data_in_audio(cover_audio, data_to_hide)
        
        # 5. Save History (Optional - keep or remove if you want)
        try:
             # Just saving the title, not the heavy encrypted content to save DB space/time
        ...
        # 6. DIRECT DOWNLOAD (The "Classic" Way)
        # We convert the bytes to a memory stream and send it immediately
        return send_file(
            io.BytesIO(stego_bytes),
            as_attachment=True,
            download_name=f"secure_{cover_file.filename}",
            mimetype='audio/wav'
        )

    except Exception as e:
        # If something breaks, go back to the page and show the error
        flash(f"Error: {str(e)}")
        return redirect(url_for('encode_page'))
