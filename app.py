...
# --- NEW OPTIMIZED DECODE FUNCTION ---
def extract_data_from_audio(audio_bytes: bytearray):
    data_start = _find_data_chunk_offset(bytes(audio_bytes))
    if data_start == -1: return None
    
    # 1. Check if file is big enough for SYNC (32 bits)
    if len(audio_bytes) < data_start + 32: return None
    
    # 2. Extract first 32 bits to check SYNC
    sync_bits = ""
    for i in range(32):
        sync_bits += str(audio_bytes[data_start + i] & 1)
        
    target_sync = ''.join(format(b, '08b') for b in SYNC_WORD)
    if sync_bits != target_sync:
        return None # No data found (Instant return, saves CPU)

    # 3. Read Length (next 32 bits)
    if len(audio_bytes) < data_start + 64: return None
    len_bits = ""
    for i in range(32):
        len_bits += str(audio_bytes[data_start + 32 + i] & 1)
    
    try:
        payload_len = int(len_bits, 2)
    except: return None
    
    # 4. Extract only the payload
    payload_bits_count = payload_len * 8
    total_read_needed = 64 + payload_bits_count
    
    if len(audio_bytes) < data_start + total_read_needed: return None
    
    payload_bits = ""
    for i in range(payload_bits_count):
        payload_bits += str(audio_bytes[data_start + 64 + i] & 1)
...
def decode_data():
    ...
    try:
        ...
        extracted = extract_data_from_audio(audio_bytes)
